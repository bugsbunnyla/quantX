...
class FormulaInfo0:
    ...
    def reporting(self, mode="both"):
       ...
       # ---------------------------------------------------------
       # Return mode is for the multiindex as horizontal/vertical/both not for df,df_easy
       # ---------------------------------------------------------
       #if mode == "multi":
       #   return df
    ...
    def report(self, mode="both"):
        ...
        # ================= MARKET =================
        ("market", "symbol"): ("market", "symbol"),
        ("market", "price"): ("market", "price"),          # canonical
        ("market", "volume"): ("market", "volume"),
        ("market", "mkt_price"): ("market", "mkt_price"),
        ("market", "mkt_ret"): ("market", "mkt_ret"),
        ("market", "ret"): ("basic", "ret"),               # from second mapping
    ...
    FORMULA_CONFIG = {
       ...
       "hit_ratio" : { "category": "basic",  "type": "hit_ratio", "formula":  "(np.sign(preds)==np.sign(future_ret)).where(future_ret.notna()).mean()",  "depends": ["preds", "future_ret"],  "dtype": "Series",
    "return_result":  "hit_ratio"},
       "preds" : { "category": "basic", "type": "preds", "formula": "signal",  "depends": ["signal"], "dtype": "array", "return_result": "preds"},
       "candidate_signal" : { "category" : "basic", "type": "candidate_signal", 
                              "formula" : "0.4*ret.rolling(30).mean()+0.2*(price/price.rolling(100).mean()-1)-0.2*ret.rolling(20).std()+0.2*(volume/volume.rolling(30).mean())",   
                              "depends":["ret","price", "volume"], "dtype" : "Series", "return_result":"candidate_signal"},
       # -------------------------
       # Time transforms
       # -------------------------
       ...
       "weight": {"category": "portfolio", "type": "weight", "formula": "pd.Series(np.linalg.solve(cov_matrix.values + 0.1*np.eye(len(cov_matrix)), ret.mean(axis=0).values), index=cov_matrix.columns)",
  "depends": ["cov_matrix", "ret", "risk_aversion"],  "dtype": "Series",  "return_result": "weight"},
       ...
       "executed_weight":{"category":"portfolio","type":"executed_weight","formula":"pd.DataFrame([norm_weight]*len(ret),index=ret.index,columns=ret.columns).shift(1).fillna(0)",
"depends":["norm_weight","ret"],"dtype":"DataFrame","return_result":"executed_weight"},
       "strategy_ret":{"category":"portfolio","type":"strategy_ret","formula":"(ret.mul(executed_weight,axis=1)).sum(axis=1)",
"depends":["executed_weight","ret"],"dtype":"DataFrame","return_result":"strategy_ret"},    
       ...
       "liquidity_adj_vol": { "category": "market_structure", "type": "liquidity_adj_vol", "formula": "volatility/(np.log(volume.mean(axis=1)+1))", "depends": ["volatility", "volume"], "dtype": "Series",  "return_result": "liquidity_adj_vol"},


       # =========================
       # EXECUTION
       # =========================
       "slippage": { "category": "execution",  "type": "slippage",  "formula": "0.5*(mkt_price_high-mkt_price_low)/mkt_price",
                     "depends": ["mkt_price", "mkt_price_high", "mkt_price_low"],  "dtype": "Series",  "return_result": "slippage"},
       "impact": { "category": "execution",  "type": "impact",   "formula": "slippage*(norm_weight.abs()**2).sum()",
  "depends": ["slippage", "norm_weight"],
                   "dtype": "Series",  "return_result": "impact"},
       "turnover": {  "category": "execution",  "type": "turnover",   "formula": "executed_weight.sub(executed_weight.shift(1)).abs().sum(axis=1)",
  "depends": ["norm_weight"],  "dtype": "Series","return_result": "turnover"},
       "pturnover": {  "category": "execution",  "type": "pturnover",   "formula": "executed_weight.diff().abs().sum(axis=1)",
  "depends": ["norm_weight"],  "dtype": "Series","return_result": "pturnover"},

       "transaction_cost":{"category":"execution", "type":"transaction_cost", "formula":"pturnover *(0.001+ret.rolling(20).std().mean(axis=1)) * TCOST / 100.0",
                           "depends":["pturnover", "ret"], "dtype":"Series",    "return_result":"transaction_cost"},
       "net_ret":{"category":"portfolio", "type":"net_ret", "formula":"strategy_ret.sub(transaction_cost, axis=0)",
...
class FormulaInfo:
    ...
    FORMULA_CONFIG = {
        ...
        "transaction_cost": {"category": "execution", "type": "transaction_cost", "formula": "pturnover *(0.001 + ret.rolling(20, min_periods=1).std().mean(axis=1).fillna(0))*TCOST/100.0", "depends": ["pturnover", "ret"], "dtype": "Series", "return_result": "transaction_cost"},
