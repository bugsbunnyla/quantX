...
class FormulaInfo:
    ...
    FORMULA_CONFIG = {
        ...
        "ret": {"category": "basic", "type": "ret", "formula": "pd.DataFrame(mkt_price.pct_change().replace([np.inf, -np.inf], np.nan).fillna(0), index=mkt_price.index, columns=mkt_price.columns)", "depends": ["mkt_price"], "dtype": "DataFrame", "return_result": "ret"},
        ...
        "strategy_ret": {"category": "portfolio", "type": "strategy_ret",  "formula": "pd.DataFrame((ret.mul(executed_weight, axis=1)).sum(axis=1), index=ret.index)", "depends": ["executed_weight", "ret"], "dtype": "DataFrame", "return_result": "strategy_ret"},
    ...
    "depends": ["daily_ret", "volume"], "dtype": "Series", "return_result": "liquidity_adj_vol"},
        ...
        "net_ret": {"category": "portfolio", "type": "net_ret",  "formula": "pd.DataFrame(strategy_ret.sub(transaction_cost, axis=0), index=strategy_ret.index)", "depends": ["strategy_ret", "transaction_cost"], "dtype": "DataFrame", "return_result": "net_ret"},
        "daily_ret": {"category": "portfolio", "type": "daily_ret", "formula": "pd.DataFrame(net_ret.copy(), index=net_ret.index)", "depends": ["net_ret"], "dtype": "DataFrame", "return_result": "daily_ret"},

        "future_ret": {"category": "intel", "type": "future_ret", "formula": "pd.DataFrame(ret.shift(-1), index=ret.index, columns=ret.columns)", "depends": ["ret"], "dtype": "DataFrame", "return_result": "future_ret"},
